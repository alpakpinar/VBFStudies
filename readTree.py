...
def drawCutFlow(inputFile):
    ...
    print('Number of events after MET > {0} cut                        : {1:>6}'.format(cuts[-1], eventCounts[1]))
    print('Number of events after nJet > 2 cut                         : {0:>6}'.format(eventCounts[2]))
    print('Number of events after jet_pt[0] > {0} cut                  : {1:>6}'.format(cuts[1], eventCounts[3]))
    print('Number of events after jet_pt[1] > {0:<3} cut                  : {1:>6}'.format(cuts[2], eventCounts[4]))
    print('Number of events after mjj > {0} cut                        : {1:>6}'.format(cuts[0], eventCounts[5]))
    print('Number of events after minPhi_jetMET > 0.5 cut              : {0:>6}'.format(eventCounts[6]))
    print('Number of events after jet_eta[0]*jet_eta[1] < 0 cut        : {0:>6}'.format(eventCounts[7]))
    print('Number of events after abs(jet_eta[0]-jet_eta[1]) > 2.5 cut : {0:>6}'.format(eventCounts[8]))
    print('Number of events after b-jet veto                           : {0:>6}'.format(eventCounts[9]))
    print('Number of events after lepton veto                          : {0:>6}'.format(eventCounts[10]))
    print('Number of events after photon veto                          : {0:>6}'.format(eventCounts[11]))
    print('*'*29)
